    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let network = tauri::async_runtime::block_on(async {
                P2pNetwork::new(profile.peer_id()).await.unwrap()
            });
...
mod commands {
    ...
    #[tauri::command]
    pub async fn get_profiles(state: State<'_, AppState>) -> Result<Vec<profile::PublicProfile>, String> {
        let network = state.network.lock().await;
        network.discover_profiles().await.map_err(|e| e.to_string())
    }

    #[tauri::command]
    pub async fn send_like(state: State<'_, AppState>, peer_id: String) -> Result<bool, String> {
        let mut network = state.network.lock().await;
        network.send_like(&peer_id).await.map_err(|e| e.to_string())
    }

    #[tauri::command]
    pub async fn open_chat(state: State<'_, AppState>, peer_id: String) -> Result<(), String> {
        let mut network = state.network.lock().await;
    ...
    pub async fn send_message(
        ...
        let mut network = state.network.lock().await;
        network.send_chat_message(&peer_id, &content).await.map_err(|e| e.to_string())?;

        let reply_msg = format!("Écho: {}", content);
    ...
    #[tauri::command]
    pub async fn close_chat(state: State<'_, AppState>, peer_id: String) -> Result<(), String> {
        let mut network = state.network.lock().await;
    ...
    #[tauri::command]
    pub async fn get_battery_status(state: State<'_, AppState>) -> Result<f32, String> {
